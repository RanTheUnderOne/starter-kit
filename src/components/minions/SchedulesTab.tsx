...
export function SchedulesTab({ agentId }: { agentId: string }) {
  ...
  return <div className="mx-auto w-full max-w-6xl p-4 md:p-8">
    ...
    <ConfirmDialog open={pendingDelete !== null} onOpenChange={(open) => !open && setPendingDelete(null)} title="Delete this schedule?" description={pendingDelete ? `“${pendingDelete.name}” and its history will be permanently deleted.` : undefined} confirmText="Delete" destructive onConfirm={async () => { try { if (pendingDelete) await scheduleState.deleteSchedule(pendingDelete); toast.success("Schedule deleted"); } catch (error) { throw new Error(sanitizeScheduleError(error)); } }} />
...
function sanitizeScheduleError(error: unknown) {
  const message = error instanceof Error ? error.message : "Something went wrong.";
  return message.replace(/https?:\/\/\S+/gi, "[internal service]").replace(/(authorization|x-agent37-key)\s*[:=]\s*\S+/gi, "$1: [redacted]");
}

function showError(error: unknown) {
  toast.error(sanitizeScheduleError(error));
}
