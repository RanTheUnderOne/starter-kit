...
p.write_text("".join(f"{key}={value}" + chr(10) for key, value in existing.items()))
os.chmod(p, 0o600)
PY`;
}
...
export async function saveOwnerPhone(db: DB, agentId: string, phone: string) {
  ...
  if (!data?.length) throw new ApiError(404, "not_found", "WhatsApp connection not found");
  return parsed;
}
...
export async function configureSharedWhatsApp(db: DB, agentId: string) {
  ...
  if (!cloud) {
    throw new ApiError(503, "config_error", "WhatsApp is not ready yet");
  }

  const vars: Record<string, string> = {
    WHATSAPP_CLOUD_ALLOWED_USERS: parsed.waId,
    WHATSAPP_CLOUD_ALLOW_ALL_USERS: "false",
    WHATSAPP_CLOUD_PHONE_NUMBER_ID: cloud.phoneNumberId,
    WHATSAPP_CLOUD_ACCESS_TOKEN: cloud.accessToken,
    WHATSAPP_CLOUD_APP_SECRET: cloud.appSecret,
    WHATSAPP_CLOUD_VERIFY_TOKEN: cloud.verifyToken,
  };
